...
| Phase | Scope | State |
|---|---|---|
| 0 | Foundations — crypto, packet primitives, tick scheduler | **in progress** |
...
- [x] `WowEmu.Protocol` — `PacketReader`/`PacketWriter`
- [ ] `WowEmu.Protocol` — packed GUID / XYZ / time (Phase 3 needs these)
- [ ] `WowEmu.Core` — `ObjectGuid`, `Position`, millisecond clock with wraparound
- [ ] Tick-bound scheduler
- [ ] SFMT-19937 + distributions
...
```
src/WowEmu.Core/           TextTransform (Latin-only uppercasing) and, later, other primitives
src/WowEmu.Cryptography/   SRP6, RC4, header encryption, session key expansion
src/WowEmu.Protocol/       PacketReader / PacketWriter
...
tools/vectors/             Python reference implementation + golden test vectors
...
| `vectors: regenerate` | Re-runs the Python reference implementation |
